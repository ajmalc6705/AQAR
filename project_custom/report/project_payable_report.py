...
class LncProjectPayableReport(models.AbstractModel):
    _name = "report.project_custom.report_project_payable_xls.xlsx"
    _inherit = "report.report_xlsx.abstract"
    _description = "LNC Project Payable Report"
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        sheet.merge_range(2, 0, 2, col_len, 'PAYABLE REPORT', format1)
        sheet.set_column(0, 0, 30)
        sheet.set_column(1, col_len - 3, 20)
        sheet.set_column(col_len - 2, col_len, 15)
        sheet.set_row(0, 50)
        sheet.set_row(2, 25)
        sheet.set_row(4, 25)
        ...
        if len(data['form']['project_ids']) != 1:
            ...
            col += 1
        sheet.write(row, col, 'Supplier', format5)
        ...
        sheet.write(row, col + 7, 'Total Payable', format5)
        ...
        for project in data['project_list']:
            ...
            if len(data['form']['project_ids']) != 1:
                ...
                col += 1
            for payable in project['payables']:
                if len(payable['lines']) > 1:
                    sheet.merge_range(row, col, row + len(payable['lines']) - 1, col, payable['name'], format3)
                    sheet.merge_range(row, col + 7, row + len(payable['lines']) - 1, col + 7, payable['total'], format8)
                else:
                    sheet.write(row, col, payable['name'], format3)
                    sheet.write(row, col + 7, payable['total'], format8)
