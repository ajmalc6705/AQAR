...
class TaxReport(models.TransientModel):
    _name = "soa.confirmation.report.wiz"
    ...
    def button_print_report(self):
        ...
        if self.auditor_id:
            datas['auditor_id'] = self.auditor_id.id
        if self.date:
        ...
        return self.env.ref('atheer_soa_report.action_balance_confirmation_document_pdf').report_action(None, data=datas)


class BalanceConfirmationReport(models.AbstractModel):
    _name = "report.atheer_soa_report.balance_confirmation_document"
    @api.model
    def _get_report_values(self, docids, data=None):
        ...
        if data.get('balance_as_of_date'):
            ...
            data['balance_as_of_date'] = datetime.strptime(balance_as_of_date, "%Y-%m-%d").strftime("%d %B %Y")
        else:
            balance_as_of_date = fields.Date.today().strftime("%d %B %Y")
            data['balance_as_of_date'] = balance_as_of_date

        if data.get('date'):
            date = data.get('date')
            data['date'] = datetime.strptime(date, "%Y-%m-%d").strftime("%d %B %Y")
        else:
            date = fields.Date.today().strftime("%d %B %Y")
            data['date'] = date
        if data.get('auditor_id'):
            auditor_id = data.get('auditor_id')
            data['auditor_id'] = self.env['res.partner'].browse(auditor_id)
        ...
        # data['total_due'] = round(abs(sum(move_ids.mapped('amount_residual_signed'))), 3)
        return {
            'doc_ids': docids,
