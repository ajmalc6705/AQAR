...
class HrOverTime(models.Model):
    ...
    # New Fields
    category = fields.Selection([('employee','Employee'),('contract_employee','Contract Employee')],string='Category')
    ...
    def create(self, values):
        seq = self.env['ir.sequence'].next_by_code('hr.overtime.request') or '/'
        values['name'] = seq
    ...
    def _onchange_date(self):
        ...
        if self.contract_id and self.date_from and self.date_to:
            for leaves in self.contract_id.resource_calendar_id.global_leave_ids:
                ...
                date_difference = (leaves.date_from - leaves.date_to).days
                leave_dates = [leaves.date_from + timedelta(days=i) for i in range(date_difference + 1)]
                # overtime_dates = pd.date_range(self.date_from, self.date_to).date
                overtime_date_difference = (self.date_from - self.date_to).days
                overtime_dates = [leaves.date_from + timedelta(days=i) for i in range(date_difference + 1)]
