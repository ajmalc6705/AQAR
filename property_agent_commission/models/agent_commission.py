...
# access_agent_payment_line,access.agent.payment.line,model_agent_payment_line,base.group_user,1,1,1,1

class AgentCommissionDetails(models.Model):
    _name = 'agent.commission'
    _description = 'Agent Commission'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'image.mixin', 'analytic.mixin']

    name = fields.Char(string='Sequence', copy=False, readonly=True, help="Sequence for Agent Payment Sequence",
                       index=True, default=lambda self: _('New'))
    partner_id = fields.Many2one('res.partner', string='Agent', domain=[('is_property_agent', '=', True)])
    issued_date = fields.Date('Issued On')

    commission_amount = fields.Float(string='Commission Amount', help="Commission percentage of total amount")

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    state = fields.Selection(selection=COMMISSION_STATE_SELECTION, string="Status", default='draft',
                             readonly=True, copy=False, tracking=True,
                             )
    commission_line_ids = fields.One2many('agent.commission.line', 'agent_commission_id')

    # commission_line_ids = fields.
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'agent.commission.sequence') or 'New'
        return super(AgentCommissionDetails, self).create(vals_list)
...
class AgentPaymentLine(models.Model):
    ...
    agent_commission_id = fields.Many2one("agent.commission")
    agent_service_type = fields.Selection([('property_sale', 'Property Sales'), ('property_lease', 'Property Lease'),
                                          ...
                                          string='Deal Type')
    partner_id = fields.Many2one('res.partner', related='agent_commission_id.partner_id', string='Agent',
                                 domain=[('is_property_agent', '=', True)])
    description = fields.Char('Description')
    calculation_type = fields.Selection([('percentage', 'Percentage (%)'), ('lump_sum', 'Lump Sum')],
                                        default='percentage')
    commission_amount = fields.Float('Commission', digits='Product Price')
    attachment_ids = fields.Many2many('ir.attachment', 'agent_commission_line_attachment_rel',
                                      'agent_commission_line_id', 'attachment_id', 'Attachments',
                                      help="You may attach files to this template, to be added to all "
                                           "commission created from this template")
    company_id = fields.Many2one('res.company', string='Company', related="agent_commission_id.company_id")
    currency_id = fields.Many2one('res.currency', related='company_id.currency_id')

    state = fields.Selection(related="agent_commission_id.state", string="Status",
    ...
    #                                  related="move_id.payment_state", store=True, readonly=True, compute_sudo=True,
    #                                  copy=False, tracking=True,
    #                                  )
    # invoice_state = fields.Selection(selection=[('draft', 'Draft'), ('posted', 'Posted'), ('cancel', 'Cancelled'), ],
    #                                  string='Status', readonly=True, copy=False, tracking=True, default='draft',
    #                                  related="move_id.state", compute_sudo=True, store=True,
    #                                  )
