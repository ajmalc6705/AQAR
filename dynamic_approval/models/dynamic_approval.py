...
class DynamicApproval(models.Model):
    ...
    submit_approve = fields.Boolean(default=False)
    ...
    def regenerate_forms(self):
        view_obj = self.env['ir.ui.view'].sudo()
        view_obj.search([('dynamic_approval_id', '=', self.id)]).unlink()
        self.regenerate_form_arch()
        self.generate_forms()
        return {
    ...
    def regenerate_form_arch(self):
        if self.button_to_hide:
            self.button_to_hide = self.button_to_hide.replace(' ', '')
            button_arch = ''
            self.env['ir.ui.view'].search([('dynamic_approval_id', '=', self.id)]).unlink()
            for button in self.button_to_hide.split(','):
                ...
                form_str = self.env[self.model_name].get_view(view_type='form')
                root = ET.fromstring(form_str['arch'])
                elements = root.findall(".//button[@name='{}']".format(button))
                for index, element in enumerate(elements):
                    domain = [('x_approval_done', '=', False)]
                    if element.attrib.get('modifiers'):
                        attrs = json.loads(element.attrib.get('modifiers'))
                        invisible = attrs.get('invisible')
                        if invisible:
                            domain = ['|', ('x_approval_done', '=', False)]
                            if len(invisible) > 1 and isinstance(invisible[0], list):
                                domain.append('&amp;')
                            for item in invisible:
                                ...
                                if isinstance(item, list):
                                    item = tuple(item)
                                domain.append(item)
                    button_arch += """<xpath expr="//button[@name='%s'][%s]" position="attributes">
                    <attribute name="attrs">{'invisible': %s}</attribute>
                    <attribute name="states"/>
                    </xpath>\n""" % (button, index+1, domain)
