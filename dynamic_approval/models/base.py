...
class BaseApproval(models.AbstractModel):
    ...
    def dynamic_next_approval(self, mode='approve', note=''):
        ...
        if approval:
            if not approval.submit_approve:
                groups = approval.line_ids.mapped('group_id')
                user_groups = self.env['res.groups'].search([('id','in',groups.ids)]).mapped('users')
                for user in user_groups:
                    self.env['mail.activity'].create({
                        'activity_type_id': self.env.ref('dynamic_approval.mail_activity_data_approve').id,
                        'summary': 'First Approval',
                        'user_id': user.id,
                        'res_model_id': model_id.id,
                        'res_id': self.id,
                    })
                approval.submit_approve = True
    ...
    def dynamic_action_approve(self):
        ...
        group = self.x_approval_group_id
        model_id = self.env['ir.model'].sudo().search([('model', '=', self._name)])
        user_groups = self.env['res.groups'].search([('id', 'in', group.ids)]).mapped('users')
        for user in user_groups:
            self.env['mail.activity'].create({
                'activity_type_id': self.env.ref('dynamic_approval.mail_activity_data_approve').id,
                'summary': 'Approval',
