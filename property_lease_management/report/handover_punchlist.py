...
class TurnOverXlsx(models.AbstractModel):
    _name = 'report.property_lease_management.hand_over_punch_list_excel'
    _inherit = 'report.report_xlsx.abstract'
    _description = "TurnOverXlsx"

    def generate_xlsx_report(self, workbook, data, checklist):
        """ generating Hand Over PunchList """
        ...
        sheet.merge_range('A6:F6', "Inspection Check List For Handing Over the Flat", format21_bg)
        ...
        sheet.write(8, 4, 'Not Working', format2)
        sheet.write(8, 5, 'Remarks', format2)

        sheet.freeze_panes(9, 0)

        row = 9
        for line in checklist.hand_over_checklist:
